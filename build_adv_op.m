...
Nf = length(q); %Could be replaced with Grid.Nf at least in 2d case
...
if ~strcmp(method,'gd') % If not first order Godunov-scheme
    ...
    else %2d case, stacks vector of grid spaces associated with faces
    ...
    A  = A + spdiags(dxy.*abs(q)/2,0,Nf,Nf)*(speye(Nf)...
        -spdiags(dt*abs(q)./dxy,0,Nf,Nf))*spdiags(phi_lim,0,Nf,Nf)*G;
end
